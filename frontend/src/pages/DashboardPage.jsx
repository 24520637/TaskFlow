...
export function DashboardPage() {
  ...
  return (
    <main className="app-shell">
      ...
      <section className="task-workspace"><div className="workspace-heading"><div><p className="eyebrow">{visibleTasks.length} of {tasks.length} shown</p><h2>{workspaceView === "calendar" ? "Your schedule" : "Your tasks"}</h2></div><div className="workspace-heading__tools"><span className="workspace-heading__note">Keep the next thing visible.</span><div className="workspace-switcher" role="tablist"><button className={workspaceView === "calendar" ? "is-active" : ""} onClick={() => setWorkspaceView("calendar")}>Calendar</button><button className={workspaceView === "list" ? "is-active" : ""} onClick={() => setWorkspaceView("list")}>List</button></div></div></div><TaskFilters filters={filters} categories={categories} onChange={updateFilters} onClear={() => setFilters(initialFilters)} />{error && <div className="form-error task-page-error" role="alert">{error}</div>}{isLoading ? <div className="empty-state"><span className="loading-dot">· · ·</span><p>Loading your tasks...</p></div> : workspaceView === "calendar" ? <CalendarView date={calendarDate} view={calendarView} tasks={visibleTasks} onViewChange={setCalendarView} onDateChange={setCalendarDate} onToday={() => setCalendarDate(new Date())} onSelectTask={(task) => setModalTask(task)} onCreateAt={createTaskAt} /> : <TaskList tasks={visibleTasks} categories={categories} onEdit={(task) => setModalTask(task)} onDelete={deleteTask} onComplete={completeTask} />}</section>
